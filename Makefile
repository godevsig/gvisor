...
$(KYTHE_DIR):
	...
	tar --no-same-owner -xzf "$$T" --directory "$(KYTHE_DIR)"; \
	rc=$$?; rm -f $$T; exit $$rc

$(KYTHE_FILENAME): STARTUP_OPTIONS += --bazelrc=$(KYTHE_DIR)/extractors.bazelrc
$(KYTHE_FILENAME): STARTUP_OPTIONS += --output_base=$(HOME)/.cache/bazel/output
$(KYTHE_FILENAME): $(KYTHE_DIR)
	@$(call build,--sandbox_tmpfs_path=$(HOME)/.cache/bazel/tmp
	              --override_repository=kythe_release=$(KYTHE_DIR) \
	              --define=kythe_corpus=github.com/google/gvisor \
	              --cxxopt=-std=c++17 //...)
	$(KYTHE_DIR)/tools/kzip merge --output "$@" -recursive $(patsubst %/*/extra_actions,%,$(BUILD_ROOTS))
